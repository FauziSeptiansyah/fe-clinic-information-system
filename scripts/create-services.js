...
// Helper to simulate latency and rare error
async function simulateNetwork<T>(data: T, allowError = false): Promise<T> {
  const latency = Math.floor(Math.random() * 400) + 150; // 150 - 550ms
  await new Promise((resolve) => setTimeout(resolve, latency));
  
  if (allowError && Math.random() < 0.03) {
    throw new Error("Gangguan koneksi jaringan simulasi. Silakan coba kembali.");
  }
...
export const queueService = {
  ...
  async createRegistration(data: {
    ...
    const newVisit: Visit = {
      ...
      patientAge: 30, // calculated in real-time
      doctorId: doctor.id,
      doctorName: doctor.name,
      departmentId: department.id,
      departmentName: department.name,
  ...
  async updateStatus(queueId: string, status: QueueStatus): Promise<Queue> {
    ...
    // Also update associated visit status
...
export const prescriptionService = {
  ...
  async dispense(id: string, dispensedBy: string, batchAllocations: Record<string, string>): Promise<Prescription> {
    ...
    rx.items.forEach((item) => {
      ...
      // Deduct medicine total currentStock
    ...
    auditLogService.log("DISPENSE_MEDICINE", "PHARMACY", id, `Dispensing obat untuk resep ${rx.prescriptionNumber} oleh ${dispensedBy}`);
    return simulateNetwork(rx);
  },
};
...
export const medicineService = {
  ...
  async update(id: string, data: Partial<Medicine>): Promise<Medicine> {
    ...
    auditLogService.log("UPDATE_MEDICINE", "INVENTORY", id, `Mengubah obat: ${medicines[index].name}`);
...
export const purchaseService = {
  ...
  async create(data: {
    ...
    // Auto-update inventory batches and stock
...
export const auditLogService = {
  ...
  log(action: string, module: string, recordId: string, description: string, user?: { id: string; name: string; role: any }) {
...
fs.writeFileSync(path.join(process.cwd(), "src/services/index.ts"), content.trim() + "\n", "utf8");
console.log("Successfully created src/services/index.ts");
